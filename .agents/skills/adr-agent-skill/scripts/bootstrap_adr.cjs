  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    if (a === '--repo-root') out.repoRoot = next();
      ...
      process.stdout.write(
        [
          'Usage: node bootstrap_adr.cjs [options]',
          '',
          'Options:',
          ...
          '  --strategy auto|date|slug|id  Filename strategy for initial ADR (default: id)',
...
function describeFilenameConvention(strategy) {
  if (strategy === 'date') {
    return 'Use date-prefixed files: `YYYY-MM-DD-choose-database.md`';
  }
  if (strategy === 'slug') {
    return 'Use slug-only files: `choose-database.md`';
  }
  return 'Use numbered files: `ADR-001-choose-database.md`';
}

function loadReadmeTemplate(strategy) {
  ...
  return fs
    ...
    .replaceAll('{FILENAME_CONVENTION}', describeFilenameConvention(strategy));
}

function writeIndex(indexFile, adrDirName, { force, strategy }) {
  if (fs.existsSync(indexFile) && !force) return;
  const content = loadReadmeTemplate(strategy).replaceAll('{ADR_DIR}', adrDirName);
...
function filenamePatternForStrategy(strategy) {
  if (strategy === 'date') return 'YYYY-MM-DD-title-with-dashes.md';
  if (strategy === 'slug') return 'title-with-dashes.md';
  return 'ADR-001-title-with-dashes.md';
}

function generateFirstAdr({ title, status, date, deciders, adrDir, strategy }) {
...
Conventions:
- One ADR per file, named \`${filenamePatternForStrategy(strategy)}\`
...
function main() {
  ...
  const strategy = args.strategy === 'auto' ? 'id' : args.strategy;
  const indexExistedBefore = fs.existsSync(indexFile);
  writeIndex(indexFile, args.dir, { force: args.forceIndex, strategy });
  const indexWritten =
  ...
  // Determine filename using the same strategy names as new_adr.cjs
  let firstAdrFilename;
  if (strategy === 'id') {
    firstAdrFilename = `ADR-001-${slugify(args.firstTitle)}.md`;
