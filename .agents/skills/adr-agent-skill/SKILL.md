...
2. **Choose a filename strategy.**
   - In this repo, prefer numbered ADR ids (`ADR-001-choose-database.md`).
   - If existing ADRs use date prefixes (`YYYY-MM-DD-...`) or slug-only filenames (`choose-database.md`), continue that convention instead of mixing formats.
...
7. **Generate the file.**
   - Preferred: run `scripts/new_adr.js` (handles directory, naming, optional category folders, and optional index updates).
...
2. **Reference the ADR in PRs.** Link to the ADR in PR descriptions, e.g. "Implements `contributing/decisions/ADR-001-use-sqlite-for-test-database.md`."
...
Numbering stays global across categories. Choose a categorization scheme early (by layer, by domain, by team) and document it in the index.
...
# Category-scoped ADR
node /path/to/adr-skill/scripts/new_adr.js --title "Choose Terraform state backend" --category infrastructure --update-index

# Bootstrap a new repo
node /path/to/adr-skill/scripts/bootstrap_adr.js --dir docs/decisions --strategy id
```

Notes:

- Scripts auto-detect ADR directory and filename strategy.
- Use `--dir`, `--category`, and `--strategy` to override.
